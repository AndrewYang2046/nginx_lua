...
--代理地址及key值定义
local CashierProxy_a = "172.28.247.111:8084"
local CashierProxy_b = "172.28.250.2:8084"
local CashierProxy_a_key = "SAND_GRAY_CHANNEL_TOPIC_SMP-PAY-CLIENT01"
local CashierProxy_b_key = "SAND_GRAY_CHANNEL_TOPIC_SMP-PAY-CLIENT02"
...
--获取数据
local resp_a, err = red:get(CashierProxy_a_key)
...
local resp_b, err = red:get(CashierProxy_b_key)
...
if grayTagA == 'gray' then
    if cookieName == 'gray' then
        if sysAddrA == CashierProxy_a then
            ngx.exec('@CashierProxy_a')
        end
    else
        ngx.exec('@CashierProxy_b')
    end
end

if grayTagB == 'gray' then
    if cookieName == 'gray' then
        if sysAddrB == CashierProxy_b then
            ngx.exec('@CashierProxy_b')
        end
    else
        ngx.exec('@CashierProxy_b')
